...
loadFiles <- function(filepath, extension = ".txt"){
        ...
        for (j in c(2,4)){
                ...
                for (i in 1:length(files))  {
                        ...
                        assign(VarName,readLines(file.path(filepath[j],files[i])), pos=1)
...
features <- read.delim(file.path(filepath,"features.txt"),sep=" ", header=FALSE, col.names=c("Id","feature"))
features <- tbl_df(features)
activitylabels <- read.delim(file.path(filepath,"activity_labels.txt"),sep=" ", header=FALSE, col.names =c("Id","activity"))
